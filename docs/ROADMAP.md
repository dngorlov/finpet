...
|---|---|---|---|---|
| 1 | Обед (lunch) | mandatory | 12 | Сытость +10 and Настроение +5; buying it cancels both daily drops |
...
- **Meters (the "key status indicators"):** Сытость and Настроение, 0–100, rendered as icon + bar (never color alone).
  ...
  - Настроение: +5 from Обед, plus the item's number from every other purchase (including a Цель bought from Копилка or Баланс). Every Игровой день ends at −15 unless a Магазин purchase that feeds Настроение cancelled it. A Цель bought from Копилка does not cancel that drop. −5 when actual optional spending exceeds the plan bucket, and that stacks with the daily drop. A smile marks the meter. Reaching the Цель price does not move Настроение.
...
- Help re-access: Словарик is a button on Карта. Tab «Слова» is the 10 terms with kid definitions: Баланс, Копилка, Цель, План, Обязательные расходы, Желаемые расходы, Сытость, Настроение, Этап, Игровой день (one-to-one with `CONTEXT.md`). Tab «Уроки» is a compact list of open уроки, all visible at once. Tapping one opens its unscored cards, with `{pet}` replaced, and no coins, questions, «скоро», or correction tasks. Closed уроки are absent. Демо-режим lists every playable урок. No replay button.
