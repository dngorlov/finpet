...
- Help re-access: Словарик is a button on Карта. Tab «Слова» is the 11 terms with kid definitions: Баланс, Копилка, Цель, Пособие, План, Обязательные расходы, Желаемые расходы, Сытость, Настроение, Этап, Игровой день (one-to-one with `CONTEXT.md`). Tab «Уроки» is a compact list of open уроки, all visible at once. Tapping one opens its unscored cards, with `{pet}` replaced, and no coins, questions, «скоро», or correction tasks. Closed уроки are absent. Демо-режим lists every playable урок. No replay button.
