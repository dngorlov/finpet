...
**4. Main (hub).** *Purpose:* everything required visible at once (R3). *Zones, top→bottom:* pet view (pose by meters: happy/idle/sad) + Этап badge; Забота and Настроение bars (icon + bar + number); coin row (Баланс badge, Копилка badge with mini progress toward Цель); active-goal card (name, accumulated/cost, remaining); active-task card (title + «Играть»); Пособие ribbon when credited today; nav grid 2×3 (План, Магазин, Копилка, Задания, Прогресс, Взрослый раздел); «Закончить день» button; ⚙ icon → Settings. *States:* plan not confirmed → План tile highlighted with hint «Составь план дня», «Закончить день» prompts to plan first; plan confirmed → tile badge «План готов»; normal play before midnight → banner «Новый день откроется завтра», economy tiles disabled with explanation, Задания replays and Словарик stay open (no dead end); Демо-режим → banner «Демо: дни идут подряд» + «Следующий день» always active.
