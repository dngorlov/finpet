...
- Первый запуск is ordered Питомец → Имя. Appearance and the pet name remain an in-memory draft until valid Имя «Дальше»; that writes the Профиль ребёнка (leftover persistence `name` equals `petName`) and opens Стартовый бюджет.
- «Как играть» is a dim/spotlight overlay after tutorial Пособие «Дальше» (or immediately if that card is not shown). Tooltip sits next to the spotlighted control; the hub scrolls that control to the center and the child does not pan. Overlay «Назад» / «Пропустить» / destination «Дальше» stay at the top. Hub beats (План, Магазин, Копилка) open when the child taps the tile — overlay «Дальше» is hidden and does not teleport. Destination beats keep overlay «Дальше». Copy is anonymous, not pet speech. «Назад» reverses; «Пропустить» is on every beat. Practice does not debit, lock a plan, or run a Задание.
- Help re-access: Словарик screen = «Как играть» (same overlay, no profile write) + the 11 terms with kid definitions: Баланс, Копилка, Цель, Пособие, План, Обязательные расходы, Желаемые расходы, Забота, Настроение, Этап, Игровой день (one-to-one with `CONTEXT.md`).
...
```
Launch
 ├─ first run ──→ FirstRun [Питомец → Имя] ─→ StartingBudget ─┐
 └─ returning ──────────────────────────────────────────────────→ Main ◄─────┘
...
**1–2. FirstRun.** *Purpose:* let the child make the pet theirs before the live hub is explained, without writing a partial profile (R1, R2). One controlled journey owns an in-memory draft through phases Питомец and Имя. *Питомец:* a complete default pet is selected; live preview plus bead sliders for «Вид» (3), «Окрас» (3), and «Аксессуар» (3); the selected stop is the accent bead in its circle, not a Chip check. *Имя:* no on-screen title; compact speech cloud above a centered pet, tail pointing down; the cloud is «Меня зовут» plus a name-tag chip (placeholder «____», trailing pencil pictogram, accessible name «Меня зовут») that accepts 1–20 visible characters after trimming; «Дальше» stays disabled until that name is valid; disabled primary is flat grey. Visible and Android Back traverse the draft without losing choices; Back from Питомец may exit, and reopening restarts from defaults. Valid Имя «Дальше» atomically creates the profile (leftover persistence `name` equals `petName`, both the trimmed pet name), then leaves for StartingBudget; on failure the draft remains and «Не получилось начать игру. Попробуй ещё раз.» offers retry. No profile row is written earlier. On «Имя», TalkBack names the chip «Меня зовут» and the pet image describes appearance without a name.

**Как играть (overlay, after tutorial Пособие).** *Purpose:* show План, Магазин, Копилка, and Задание on the real screens (R1, R11). The rest of the screen is dimmed; one control is spotlighted and scrolled to the center (the child does not pan); a short anonymous tooltip next to it (not the Питомец, not Помощник) explains it. Overlay «Назад» / «Пропустить» / destination «Дальше» stay in a bar at the top. Hub tiles open by tap; overlay «Дальше» is hidden on those beats and does not teleport. Destination beats (buckets, shop, Положить, Задание) keep overlay «Дальше». «Назад» reverses (beat 1 and Android Back skip); «Пропустить» dismisses. Shop may open Обед; Купить, «Подтвердить план», Положить, Играть, and Закончить день do not write and the action buttons stay disabled. Last «Дальше» or skip marks the walkthrough done and leaves Main. Replay from Словарик starts the same overlay without a second Пособие card and without a profile write. TalkBack names the tooltip and the overlay actions; on a hub beat the spotlighted tile is the way forward.
...
**4. Main (hub).** *Purpose:* everything required visible at once (R3). *Zones, top→bottom:* pinned status strip shared with other play screens (Забота icon+bar, Настроение icon+bar, Баланс 🪙+number, Этап three DVD dots then current name, ⚙ → Settings with accessible name «Настройки»); large centered pet (pose by meters: happy/idle/sad); chunky Забота and Настроение MeterBars (icon + label + bar + number); Пособие ribbon when credited today; Card for active Цель (name, accumulated/cost, remaining); Card for active Задание (title + «Играть»); 2×3 NavTiles (pictogram + word: План, Магазин, Копилка, Задания, Прогресс, Взрослый раздел) — Копилка tile also shows the pot number; raised «Закончить день». The strip is hidden while «Как играть» is up. Plan-not-confirmed highlight on the План tile includes a check plus «Составь план дня», never color alone. *States:* plan not confirmed → План tile highlighted with hint «Составь план дня», «Закончить день» prompts to plan first; plan confirmed → tile badge «План готов»; normal play before midnight → banner «Новый день откроется завтра», economy tiles disabled with explanation, Задания replays and Словарик stay open (no dead end); Демо-режим → banner «Демо: дни идут подряд» + «Следующий день» always active.

**5. Plan (План).** *Purpose:* promise today's split across 3 areas without moving coins (R5). *Zones:* available amount header (balance incl. today's Пособие); promise line «Это обещание на сегодня. Монеты пока в Балансе.»; three bucket rows — Обязательные, Желаемые, Копилка — each with the integer, a horizontal track 0…available, −/+ (tap ±1, hold repeats), and a pictogram; Копилка extra «Положишь их отдельно — в Копилке.»; on later days a draft-only «вчера N» per row from last closed actuals (purchases / `savings_in`), including «вчера 0»; remainder line «Останется свободных: N»; validation line when total > available (blocks confirm); «Подтвердить план». After confirm, leftover follow-through is on Магазин (open tab) and Копилка home, not on this screen, Main, or the status strip. *States:* draft (editable; yesterday hint when a closed day exists) → confirmed (locked; confirmation sheet repeats that this is a promise: coins stay in Баланс until Магазин / Копилка, then the plan cannot change; no yesterday; no track) → during-day view adds plan-vs-actual columns (план / потрачено per bucket) → day closed (read-only). *Leaves:* Main.
